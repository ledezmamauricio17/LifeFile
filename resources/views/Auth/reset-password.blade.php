...
<div class="w-full">
    <div class="flex mt-3 ml-4 " id="password">
        <a href="{{ asset('confirm-password') }}" class="flex items-center">
    ...
    <form class="justify-center mt-6 items-center w-4/5 border-primary border shadow rounded-2xl bg-white px-6 flex flex-col md:w-1/2 lg:w-1/3 m-auto" action="{{ route('password.update')}}" method="post">
        @csrf
        <div class="grid mb-3 pt-3" id="newpaswword">
            <h1 class="w-full text-2xl font-semibold leading-tight text-center text-black">Nueva contraseña</h1>
        </div>
        <div class="w-full p-2 justify-start flex flex-col">
            ...
            @if (Auth::user()== null)
                @foreach ($email as $em)
                    <input class="hidden" name="email" type="email" value="{{ $em['email'] }}"/>
                @endforeach
            @endif
            <div class="my-4 flex flex-row">
                ...
                <input type="password" autofocus class="h-10 border border-gray-200 rounded-r-lg outline-none focus:ring-1 ring-primary w-full pl-1" name="password" placeholder="Nueva password">
            </div>
            <div class="my-4 mt-5 flex flex-row">
                <span class="z-highest rounded-l-lg w-10 h-10 flex justify-center items-center text-2xl text-gray-400 border border-r-0" mode="render" block="">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32" class="iconify iconify--carbon text-gray-500">
                        <path d="M21 2a8.998 8.998 0 0 0-8.612 11.612L2 24v6h6l10.388-10.388A9 9 0 1 0 21 2zm0 16a7.013 7.013 0 0 1-2.032-.302l-1.147-.348l-.847.847l-3.181 3.181L12.414 20L11 21.414l1.379 1.379l-1.586 1.586L9.414 23L8 24.414l1.379 1.379L7.172 28H4v-3.172l9.802-9.802l.848-.847l-.348-1.147A7 7 0 1 1 21 18z" fill="currentColor">
                        </path>
                ...
                <input type="password" autofocus class="h-10 border border-gray-200 rounded-r-lg outline-none focus:ring-1 ring-primary w-full pl-1" name="password_confirmation" placeholder="Confirmar password">
            </div>
            @error('password')
            ...
            <div class="text-center">
                <button type="submit" class="py-2 rounded bg-primary text-black hover:bg-blue-500 hover:text-white my-4 w-2/5">Guardar</button>
            </div>
        </div>
    </form>
</div>
